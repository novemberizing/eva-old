...
int main(int argc, char ** argv)
{
    ...
    const char * reject = "12e12345678901234567890";
    ...
    z.i128 = _mm_cmpestrm(x.i128, 16, y.i128, 16, _SIDD_UBYTE_OPS | _SIDD_CMP_EQUAL_ANY);
    for(int i = 0; i < 4; i++)
    {
        printf("%08x\n", z.i32[i]);
    }

    z.i128 = _mm_cmpestrm(x.i128, 16, y.i128, 16, _SIDD_UBYTE_OPS | _SIDD_CMP_RANGES);
    for(int i = 0; i < 4; i++)
    {
        printf("%08x\n", z.i32[i]);
    }

    z.i128 = _mm_cmpestrm(x.i128, 16, y.i128, 16, _SIDD_UBYTE_OPS | _SIDD_CMP_EQUAL_EACH);
    for(int i = 0; i < 4; i++)
    {
        printf("%08x\n", z.i32[i]);
    }

    z.i128 = _mm_cmpestrm(x.i128, 16, y.i128, 16, _SIDD_UBYTE_OPS | _SIDD_CMP_EQUAL_ORDERED);
    for(int i = 0; i < 4; i++)
    {
        printf("%08x\n", z.i32[i]);
    }
