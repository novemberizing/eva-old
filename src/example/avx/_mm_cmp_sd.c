...
int main(int argc, char ** argv)
{
    ...
    vector128 x = { .f64 = { nan64, 1.0f } };
    vector128 y = { .f64 = { 1.0f, 0.0f } };
    vector128 z = { 0, };

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_LT_OQ);
    ...
    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NGE_UQ);
    ...
    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_LE_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf <= %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NGT_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf <= %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_EQ_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf == %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_EQ_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf == %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }
    ...
    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NEQ_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf != %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NEQ_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf != %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_GE_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf >= %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NLT_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf >= %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_GT_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf > %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_NLE_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf > %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_ORD_Q);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf > %lf true %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_TRUE_UQ);
    for(int i = 0; i < 2; i++)
    {
        printf("unordered: %lf true %lf = %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_FALSE_OQ);
    for(int i = 0; i < 2; i++)
    {
        printf("ordered: %lf > %lf false %s %016lx\n", x.f64[i], y.f64[i], z.u64[i] ? "true" : "false", z.u64[i]);
    }

    z.d128 = _mm_cmp_sd(x.d128, y.d128, _CMP_UNORD_Q);
    for(int i = 0; i < 2; i++)
    {
