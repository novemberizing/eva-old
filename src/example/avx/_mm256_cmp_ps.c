...
int main(int argc, char ** argv)
{
    ...
    vector256 x = { .f32 = { nan64, 0.0f, 1.0f, 2.0f, nan64, 0.0f, 1.0f, 2.0f } };
    vector256 y = { .f32 = {  1.0f, 1.0f, 1.0f, 1.0f,  1.0f, 1.0f, 1.0f, 1.0f } };
    vector256 z = { 0, };

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_LT_OQ);
    ...
    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NGE_UQ);
    ...
    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_LE_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf <= %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NGT_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf <= %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_EQ_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf == %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_EQ_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf == %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }
    ...
    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NEQ_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf != %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NEQ_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf != %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_GE_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf >= %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NLT_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf >= %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_GT_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf > %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_NLE_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf > %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_ORD_Q);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf true %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_TRUE_UQ);
    for(int i = 0; i < 8; i++)
    {
        printf("unordered: %lf true %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_FALSE_OQ);
    for(int i = 0; i < 8; i++)
    {
        printf("ordered: %lf false %lf = %s %08x\n", x.f32[i], y.f32[i], z.u32[i] ? "true" : "false", z.u32[i]);
    }

    z.f256 = _mm256_cmp_ps(x.f256, y.f256, _CMP_UNORD_Q);
    for(int i = 0; i < 8; i++)
    {
