...
/**
 * @fn          extern xint64 xsocketcreate(xsocket * o)
 * @brief       소켓을 생성합니다.
 * @details     클라이언트, 서버 소켓의 경우 이벤트를 등록해야 합니다.
 * 
 * @param       o | xsocket * | in | 소켓 객체 |
 *  
 * 
 */
extern xint64 xsocketcreate(xsocket * o)
{
    ...
    if(xsocketcheck(o, xsocketstatus_close) == xfalse)
    {
        if(o->handle.f < 0)
        {
            xassertion(xsocketcheck(o, xsocketstatus_create), "");
    ...
    // 아무것도 수행하지 않습니다. 이 함수는 호출되지 않아야 했던 것 입니다.
    xlogfunction_end("%s(...) => %ld", __func__, xfail);
    return xfail;
}
